 ...
 * ===================================================
 * zigzag.js    0.8.1207
 * jQuery zigzag Plugin (Tools) 基于jQuery的静态工具类
 * Requires jQuery v2.0 or later
 *          jQuery Form v3.50 or later
 * Copyright (c) 2014
 * 
 * Project repository: https://github.com/153734009/zigzag
 * --Dual licensed under the MIT and GPL licenses.--
 * --https://github.com/153734009/zigzag#copyright-and-license--
 * 日志：
...
;jQuery.extend({
	...
	zigzag:{
		...
		ajaxFormCheckReturn:function($form, data){
			...
			var id = $form.attr("id");
			if (typeof(data) == 'string') {
				$form.after('<label id="'+id+'-error'+'" class="text-danger" for="'+id+'">意外错误.</label>');
				console.warn(data);
				throw new Error("意外错误");
			}else if (data.errcode) {
				var str = data.errmsg+'('+data.errcode+')';
				$form.after('<label id="'+id+'-error'+'" class="text-danger" for="'+id+'">'+str+'</label>');
				console.info( str );
				throw new Error(str);
...
;(function($, window, document, undefined) {// 全局性的一些动作
	//1. form全部validate
	$.fn.validate && $("form:not([noValidate])").each(function(){
	...
	//2. form全部ajaxForm
    $.fn.ajaxForm && $("form:not([noAjaxForm])").ajaxForm({
    ...
    //5. 删除一行
	$(document).on('click', "[data-toggle='ajaxDelRow']", function(){
	//$("#slideList,#menuList").on('click', "[data-toggle='ajaxDelRow']", function(){
		if ( !confirm('确认删除？') ){
			return false;
		}
		var url	= $(this).data('action') ? $(this).data('action'):$(this).attr('href'),
			me	= $(this);
			row = $(this).data("row");
		row = typeof(row)==='undefined' ?  me.closest("tr"):me.closest(row);
		if($(this).hasClass("debug")){$.zigzag.submit(url, {});return false;}
	...
	//6. 无处理的删除
	$("[data-toggle='ajaxDel']").click(function(){
		if ( !confirm('确认删除？') ){
			return false;
		}
			url	= $(this).data('action'),
			me = $(this);
		//$.zigzag.submit(url, {});return false;
		$.post(url, {}, function(data){
			e = $.Event('finish.ajaxDel');
			e.data = data;
			me.trigger(e);
		})
	});
